...
def parseArgs():
    print("TimeBasedLoginUserEnum v1.1\n")
    ...
    parser.add_argument(
        ...
        default=None,
        required=True,
        help="File containing list of usernames to test.",
    )
    parser.add_argument(
        "-t",
        "--threads",
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Disables colored output mode.",
    )
...
if __name__ == "__main__":
    ...
    print(
        "   [>] Trying %d logins with random user '%s'"
        % (options.samples, random_username)
    )
    average_rand_user = average_response_time(
        random_username, threads=options.threads, samples=options.samples
    )
    ...
    average_real_user = average_response_time(
        options.username, threads=options.threads, samples=options.samples
    )
